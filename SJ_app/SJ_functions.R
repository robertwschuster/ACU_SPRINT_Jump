#-----------------------------------------------------------------------------------------
#
# Functions for SJ analysis Shiny app
...
perfMetrics <- function(data) {
  ...
  for (r in 1:length(reps)) {
    ...
    # impulse
    fz <- data[[rn]]$Total[sj:data$flight[r,1]] # Fz = force between start of concentric and take-off
    j <- cumsum(c(0,(fz[1:(length(fz)-1)] + fz[2:length(fz)])/2 * diff(data[[rn]]$Time[sj:data$flight[r,1]]))) # impulse (trapz integration of force)
    # power
    ...
    pm[r,25] <- FT / (data[[rn]]$Time[data$flight[r,1]] - data[[rn]]$Time[sj]) # FT:CT
    pm[r,26] <- data[[rn]]$Time[data$flight[r,1]] - data[[rn]]$Time[sj] # contraction time
